...
def plot_mesh(ax,
    ...
    if nodes_primary is None and nodes_secondary is None:
        ...
        ax.triplot(nodal_positions[:, 0], nodal_positions[:, 1], triangles=connectivity_primary, color='#0e437c', linewidth=0.25)
        ax.triplot(nodal_positions[:, 0], nodal_positions[:, 1], triangles=connectivity_secondary, color='#d55209', linewidth=0.25)
    if nodes_candidate_primary is not None:
        ax.scatter(nodal_positions[nodes_candidate_primary, 0], nodal_positions[nodes_candidate_primary, 1], s=20, facecolors='none', edgecolors='#0e437c', linewidth=0.5)
    if nodes_candidate_secondary is not None:
        ax.scatter(nodal_positions[nodes_candidate_secondary, 0], nodal_positions[nodes_candidate_secondary, 1], s=20, facecolors='none', edgecolors='#d55209', linewidth=0.5)
    if nodes_interface_primary is not None:
        ax.scatter(nodal_positions[nodes_interface_primary, 0], nodal_positions[nodes_interface_primary, 1], s=20, color='#0e437c')
    if nodes_interface_secondary is not None:
        ax.scatter(nodal_positions[nodes_interface_secondary, 0], nodal_positions[nodes_interface_secondary, 1], s=20, color='#d55209')
    if nodes_added_primary is not None:
        ax.scatter(nodal_positions[nodes_added_primary, 0], nodal_positions[nodes_added_primary, 1], s=20, facecolors='none', edgecolors='red')
    if nodes_added_secondary is not None:
        ax.scatter(nodal_positions[nodes_added_secondary, 0], nodal_positions[nodes_added_secondary, 1], s=20, facecolors='none', edgecolors='red')
    if nodes_dumped_primary is not None:
        ax.scatter(nodal_positions[nodes_dumped_primary, 0], nodal_positions[nodes_dumped_primary, 1], s=20, marker='x', color='green')
    if nodes_dumped_secondary is not None:
        ax.scatter(nodal_positions[nodes_dumped_secondary, 0], nodal_positions[nodes_dumped_secondary, 1], s=20, marker='x', color='green')
...
def write_solution(mesh, new_nodal_positions):
    ...
    while i < len(nodal_positions):
        ...
        if n_nodes_in_group == 0:
            i += 1
            continue
        nodal_positions[i+1+n_nodes_in_group:i+1+2*n_nodes_in_group] = \
