\begin{algorithmic}[1]
    ...
    \While{$c \leq C$}
        \State For each node $i$, let $r_i \gets d_i / c$ \Comment{Condition \refequ{equ:c-condition}}
        \State For each node $i$, count $n_i = \# \{j \neq i: \mathbf{D}^{\boldsymbol{\xi}, \boldsymbol{\xi}}(i, j) < r_i \}$
        \If{For all nodes $i$, $n_i < 1/\phi(c)$} \Comment{Condition \refequ{equ:support-condition}}
            \State \textbf{break}
        \EndIf
        \State Increase $c$
    \EndWhile
